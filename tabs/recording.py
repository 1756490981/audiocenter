"""Recording device tab — list capture devices, set default, volume, format, listen, enable/disable."""
...
class RecordingDeviceTab(ctk.CTkFrame):
    ...
    def _bg_refresh(self):
        ...
        while self._alive:
            ...
            if not self._alive:
                break
            try:
                dev_res = self.helper.send('list-devices', type='capture', timeout=2.0)
    ...
    def _do_refresh(self):
        try:
            dev_res = self.helper.send('list-devices', type='capture', timeout=3.0)
            pb_res = self.helper.send('list-devices', type='render', timeout=3.0)
            devices = dev_res.get('devices', [])
            playback_devices = [d for d in pb_res.get('devices', [])
                                if d.get('state') == 'active']
        except Exception:
            return
        volumes = {}
        for d in devices:
            ...
            try:
                ...
                volumes[d['id']] = {'hasEndpointVolume': False}
        self.after(0, self._apply_refresh, devices, volumes, playback_devices)

    def _apply_refresh(self, devices, volumes, playback_devices):
    ...
    def _set_default(self, device_id):
        def _run():
            try:
                self.helper.send('set-default-device', id=device_id, type='capture', timeout=3.0)
            except Exception:
                pass
    ...
    def _context_menu(self, event, device):
        ...
        if device.get('state') == 'active':
            ...
            menu.add_command(label='侦听', command=lambda: self._show_listen(device))
    ...
    def _show_format(self, device):
        ...
        # Build form (hidden until format loaded)
        ...
        ctk.CTkLabel(form, text='常用预设', font=('Microsoft YaHei UI', 11),
    ...
    def _show_listen(self, device):
        ...
        # Build form (hidden until data loaded)
        form = ctk.CTkFrame(dialog, fg_color='transparent')
        ...
        pb_names = ['默认播放设备'] + [d.get('name', '') for d in self._playback_devices]
        pb_ids = [''] + [d.get('id', '') for d in self._playback_devices]
        pb_var = ctk.StringVar(value=pb_names[0])
        ...
        def _save():
            idx = pb_names.index(pb_var.get()) if pb_var.get() in pb_names else 0
            threading.Thread(target=lambda: (
                self.helper.send('set-listen', id=device['id'],
                                 enabled=listen_enabled.get(),
                                 playback_device_id=pb_ids[idx]),
                self.after(200, self.refresh)
            ), daemon=True).start()
            dialog.destroy()
        ...
        def _fetch():
            try:
                r = self.helper.send('get-listen', id=device['id'], timeout=2.0)
            except Exception:
                r = {}
            current_playback_id = r.get('playbackDeviceId', '')
            try:
                idx2 = pb_ids.index(current_playback_id) if current_playback_id in pb_ids else 0
            except ValueError:
                idx2 = 0
            self.after(0, lambda: [
                listen_enabled.set(r.get('enabled', False)),
                pb_var.set(pb_names[idx2]),
                loading.pack_forget(),
                form.pack(fill='both', expand=True),
            ])
        threading.Thread(target=_fetch, daemon=True).start()
